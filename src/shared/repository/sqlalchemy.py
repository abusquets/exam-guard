...
from shared.repository.ports.generic import AbstractReadRepository, AbstractRepository, FilterBy
...
class SqlAlchemyReadRepository(AbstractReadRepository[EntityT]):
    entity: Type[EntityT]
    ...
    def __new__(cls, *args: Any, **kwargs: Any) -> Self:  # noqa: ARG003
        ...
        if base:
            generics: Tuple[Type[EntityT]] = base[0].__args__
    ...
    def _filter_by(self, by: FilterBy) -> Select:
        return select(self.entity).filter_by(**by)

    async def filter_by(self, by: FilterBy) -> List[EntityT]:
        query = self._filter_by(by)
        async with self.session_factory() as session:
            return list((await session.scalars(query)).all())


class SqlAlchemyRepository(SqlAlchemyReadRepository[EntityT], AbstractRepository[EntityT, CreateT, UpdateT]):
    def __new__(cls, *args: Any, **kwargs: Any) -> Self:  # noqa: ARG003
        base: Optional[Tuple[Any, ...]] = getattr(cls, '__orig_bases__', None)
        if base:
